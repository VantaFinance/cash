...
Cash Bus использует данный endpoint в качестве резервного механизма получения статуса заявки. При недоступности основного канала доставки событий система автоматически переключается на периодический опрос (polling) для синхронизации состояния заявки.
